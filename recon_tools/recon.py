...
def get_dns_records(domain):
    ...
    try:
        for record in ['A', 'MX', 'NS']:
            ...
            log_output(f"{record} Records:")
            for rdata in answers:
                log_output("   " + rdata.to_text())
    except Exception as e:
...
common_subdomains = ['www', 'mail', 'ftp', 'admin', 'test', 'portal', 'dev', 'blog', 'api']
...
def scan_subdomain(domain, subdomain):
    ...
    try:
        ...
        if response.status_code < 400:
            log_output(f"[+] Found Subdomain: {url}")
    except:
        pass
